...
wgettest()
{
	echo "===start wget test===" | tee -a $logfilename
	nohup python -m SimpleHTTPServer 8181 >/dev/null 2>/dev/null &
	wget -N http://cachefly.cachefly.net/10mb.test 2>/dev/null
	wget http://test.91yun.org/testcode/test-wget.py 1>/dev/null 2>&1
	#徐州的因为单网卡双IP单独处理
	wgetspeed=$(python test-wget.py 221.229.197.94)
	xzdxspeed=$(echo $wgetspeed | awk -F '|' '{print $1}')
	xzltspeed=$(echo $wgetspeed | awk -F '|' '{print $2}')
	echo "徐州电信："$xzdxspeed"：江苏捷联提供QQ1965516" | tee -a $logfilename
	echo "徐州联通："$xzltspeed"：江苏捷联提供QQ1965516" | tee -a $logfilename
	#单ip的可以按以下格式处理`服务器名字：代码：赞助文字`注意是中文的冒号分割，内容中不能再出现中文冒号
	echo "四川德阳移动："$(python test-wget.py 117.174.59.145)"：" | tee -a $logfilename
	echo "===wget test end===" | tee -a $logfilename
	kill $(ps aux|grep SimpleHTTPServer|awk '{print $2}') 1>/dev/null 2>&1
	rm -rf 10mb.test
}
