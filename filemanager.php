<?php
/********************************
简易PHP文件管理器 jcampbell1@github制作 singhigh@hostloc汉化

Liscense: MIT

安装方法：把本文件放到访问目录下，从Googleapis下载jquery.min.js 1.82版本（http://ajax.googleapis.com/ajax/libs/jquery/1.8.2/jquery.min.js）放在自己的网站目录下，否则wall内的用户无法列目录，按照范例更改本文件的第203行。Linux系统还需将文件所在的目录及子目录等文件更改权限或所有者到http服务器所在用户下。另外【必须】启用scandir这个PHP函数，否则无法列目录。另外，建议在根目录放个控制搜索引擎爬虫的robots.txt
********************************/

#配置字段

// 修改本字段，设定是否允许删除文件（false为不允许删除）。
$allow_delete = true;

/* 将第16-30行内容取消注释，即可增加登陆密码。修改第18行defaultpass字样修改密码。 */
...
if($_POST) {
	if($_COOKIE['_sfm_xsrf'] !== $_POST['xsrf'] || !$_POST['xsrf'])
		err(403,"身份验证失败，请联系管理员，若确认密码正确可清空Cookie后重试。");
}
...
$(function(){
	...
	function list() {
		...
		$.get('?',{'do':'list','file':hashval},function(data) {
			...
			if(data.success) {
				...
				!data.results.length && $tbody.append('<tr><td class="empty" colspan=5>当前目录无文件</td></tr>')
	...
	function renderBreadcrumbs(path) {
		var base = "",
			$html = $('<div/>').append( $('<a href=#>主目录</a></div>') );
...
<footer>简易文件管理器 <a href="https://github.com/jcampbell1/simple-file-manager">jcampbell1</a> 制作</br>请使用IE10以上、Chrome或Firefox浏览器。</footer>
</body></html>
